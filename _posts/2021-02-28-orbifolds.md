...
* When $n=1$, the only non-trivial finite subgroup is $\{\pm Id \}$ which acts on $\mathbb{R}$ by reflection about the origin. The quotient $\mathbb{R}/\{\pm Id\}$ is the closed infinite ray $[0,\infty)$ where the boundary point $0$ acts as a mirror point of the action. Thus, the only possible singularity in dimension $1$ is a mirror point.
