...
Each $V \in \mathcal{B}$ is surely second-countable since it is a finite cover of an open subset of a second-countable space $\mathbb{C}$. As such, each $V$ must only intersect countably many elements of $\mathcal{B}$. We can pick an element $V$ in $\mathcal{B}$ and define $\mathcal{B}_1 = \{V\}$ and inductively $\mathcal{B}_n$ to be the collection of elements of $\mathcal{B}$ which intersect some element of $\mathcal{B}_{n-1}$. By connectivity, $\mathcal{B} = \bigcup_{n=1}^\infty \mathcal{B}_n$, but since this is a countable union of countable sets, $\mathcal{B}$ is indeed a countable basis for $Z$.
