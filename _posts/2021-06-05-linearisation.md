...
1. By the root test and by replacing $f(x)$ with $f(\epsilon x)/\epsilon$ for some $0<\epsilon<1$ if necessary, we can assume that there is some $\vert \lambda \vert \leq \tau < 1$ such that $ \vert a_m \vert \leq \vert \tau \vert^m $ for all $m \geq 2$. Since $\sum_{m=1}^{k-1} m d_m = n$, every monomial that appears in each polynomial $c_k$ is bounded by $\tau^n$.
